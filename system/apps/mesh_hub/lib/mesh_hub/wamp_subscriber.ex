defmodule MeshHub.WampSubscriber do
  @moduledoc """
  ...
  @topics [
    {"energy.home.", :prefix},        # All home events (production, consumption, storage, contract)
    {"energy.utility.", :prefix}      # All utility tariff updates
  ]
  ...
  def handle_info({:connect, bondy_url, realm_uri}, state) do
    case MeshWamp.start_link(url: bondy_url, realm: realm_uri) do
      {:ok, wamp_client} ->
  ...
  def handle_info(:subscribe_to_topics, state) do
    ...
    Enum.each(@topics, fn {topic, match_type} ->
      ...
      case MeshWamp.subscribe(state.wamp_client, topic, handler, options) do
        :ok ->

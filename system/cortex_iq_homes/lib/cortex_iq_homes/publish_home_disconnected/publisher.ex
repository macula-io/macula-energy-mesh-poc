defmodule CortexIqHomes.PublishHomeDisconnected.Publisher do
  @moduledoc """
  Publishes home.disconnected events to WAMP using the shared pool.
  """
  use GenServer
  require Logger
  ...
  def init(opts) do
    ...
    Logger.debug("PublishHomeDisconnected.Publisher: Started for home #{home_id}")
    {:ok, %{home_id: home_id, pool_name: pool_name}}
  end

  @impl true
  def handle_info({:publish, data}, state) do
    Logger.debug("PublishHomeDisconnected.Publisher: Publishing for home #{state.home_id}")
    case MaculaSdk.Wamp.Pool.publish(@topic, [], data, %{}, state.pool_name) do
      :ok -> Logger.debug("PublishHomeDisconnected.Publisher: ✓ Published")
      {:error, reason} -> Logger.error("PublishHomeDisconnected.Publisher: ✗ Failed: #{inspect(reason)}")
    end
    {:noreply, state}
  end

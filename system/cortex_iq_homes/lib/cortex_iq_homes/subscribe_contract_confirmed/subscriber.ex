defmodule CortexIqHomes.SubscribeContractConfirmed.Subscriber do
  @moduledoc """
  Subscribes to be.cortexiq.market.contract_confirmed events.

  Topic: be.cortexiq.market.contract_confirmed
  Notifies: HomeBot via {:contract_confirmed, data}
  """
  use GenServer
  require Logger
  ...
  def handle_info({:event, event_data}, state) do
    event_data
    |> Map.get(:kwargs, %{})
    |> notify_home_bot_with_data(state.home_id, :contract_confirmed)

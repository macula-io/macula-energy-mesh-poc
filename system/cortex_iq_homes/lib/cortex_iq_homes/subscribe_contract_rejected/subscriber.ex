defmodule CortexIqHomes.SubscribeContractRejected.Subscriber do
  @moduledoc """
  Subscribes to be.cortexiq.market.contract_rejected events.

  Topic: be.cortexiq.market.contract_rejected
  Notifies: HomeBot via {:contract_rejected, data}
  """
  use GenServer
  require Logger
  ...
  def handle_info({:event, event_data}, state) do
    event_data
    |> Map.get(:kwargs, %{})
    |> notify_home_bot_with_data(state.home_id, :contract_rejected)

defmodule CortexIqSimulation.ResetSimulation.System do
  @moduledoc """
  RPC System for resetting the simulation.

  Registers WAMP RPC procedure: be.cortexiq.simulation.reset
  ...
  def handle_info(:register_procedure, state) do
    with {:ok, client} <- get_client(state),
      ...
      case Client.register(client, @rpc_procedure, handler) do
        :ok ->
          Logger.info("ResetSimulation: Registered RPC procedure #{@rpc_procedure}")
          {:noreply, state}
        ...
        {:error, reason} ->
          Logger.error("ResetSimulation: Failed to register #{@rpc_procedure}: #{inspect(reason)}")
          {:noreply, state}
      end
    else
      _ ->
        Logger.warning("ResetSimulation: Session not ready, retrying in 1s...")
  ...
  defp handle_reset_rpc(state) do
    ref = make_ref()
    send(state.simulation_clock, {:rpc_reset, ref, self()})
    ...
    after
      5000 ->
        Logger.error("ResetSimulation: Timeout waiting for SimulationClock response")
        {:error, %{"error" => "timeout"}}
    end
  end
end

defmodule CortexIqSimulation.SetSimulationSpeed.System do
  ...
  def handle_info(:connect, state) do
    Logger.info("SetSimulationSpeed: Connecting to WAMP realm #{state.realm} at #{state.bondy_url}")

    case Client.start_link(url: state.bondy_url, realm: state.realm) do
      {:ok, client} ->
        Logger.info("SetSimulationSpeed: WAMP client started, waiting for session...")

defmodule CortexIqSimulation.PauseSimulation.System do
  ...
  Registers WAMP RPC procedure: be.cortexiq.simulation.pause
  ...
  def handle_info(:connect, state) do
    Logger.info("PauseSimulation: Connecting to WAMP realm #{state.realm} at #{state.bondy_url}")

    case Client.start_link(url: state.bondy_url, realm: state.realm) do
      {:ok, client} ->
        Logger.info("PauseSimulation: WAMP client started, waiting for session...")

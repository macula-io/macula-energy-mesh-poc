defmodule CortexIqProjections.ProjectHomeConnected.Subscriber do
  @moduledoc """
  Subscribes to be.cortexiq.home.connected WAMP events and forwards to projector.
  ...
  def handle_info(:subscribe, %{wamp_client: wamp_client} = state) do
    ...
    Logger.info("ProjectHomeConnected.Subscriber: Subscribing to #{topic}")

    case Client.subscribe(wamp_client, topic, &handle_event/2, %{}) do
      :ok ->
        Logger.info("ProjectHomeConnected.Subscriber: ✓ Subscribed to #{topic}")
        {:noreply, %{state | subscription_status: :subscribed}}

      {:error, reason} ->
        Logger.error("ProjectHomeConnected.Subscriber: ✗ Failed: #{inspect(reason)}")
  ...
  def handle_info(msg, state) do
    Logger.debug("ProjectHomeConnected.Subscriber: Unexpected message: #{inspect(msg)}")
    {:noreply, state}
  end
  ...
  defp handle_event(_topic, event_data) do
    CortexIqProjections.ProjectHomeConnected.Projector.project_event(event_data)
  end
end

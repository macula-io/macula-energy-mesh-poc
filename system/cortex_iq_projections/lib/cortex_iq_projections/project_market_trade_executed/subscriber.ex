defmodule CortexIqProjections.ProjectMarketTradeExecuted.Subscriber do
  @moduledoc """
  Subscribes to be.cortexiq.market.trade_executed WAMP events and forwards to projector.
  ...
  def handle_info(:subscribe, %{wamp_client: wamp_client} = state) do
    ...
    Logger.info("ProjectMarketTradeExecuted.Subscriber: Subscribing to #{topic}")

    case Client.subscribe(wamp_client, topic, &handle_event/2, %{}) do
      :ok ->
        Logger.info("ProjectMarketTradeExecuted.Subscriber: ✓ Subscribed to #{topic}")
        {:noreply, %{state | subscription_status: :subscribed}}

      {:error, reason} ->
        Logger.error("ProjectMarketTradeExecuted.Subscriber: ✗ Failed: #{inspect(reason)}")
  ...
  def handle_info(msg, state) do
    Logger.debug("ProjectMarketTradeExecuted.Subscriber: Unexpected message: #{inspect(msg)}")
    {:noreply, state}
  end
  ...
  defp handle_event(_topic, event_data) do
    CortexIqProjections.ProjectMarketTradeExecuted.Projector.project_event(event_data)
  end
end

defmodule CortexIqProjections.ProjectMarketSavingsRealized.Subscriber do
  @moduledoc """
  Subscribes to be.cortexiq.market.savings_realized WAMP events and forwards to projector.
  ...
  def handle_info(:subscribe, %{wamp_client: wamp_client} = state) do
    ...
    Logger.info("ProjectMarketSavingsRealized.Subscriber: Subscribing to #{topic}")

    case Client.subscribe(wamp_client, topic, &handle_event/2, %{}) do
      :ok ->
        Logger.info("ProjectMarketSavingsRealized.Subscriber: ✓ Subscribed to #{topic}")
        {:noreply, %{state | subscription_status: :subscribed}}

      {:error, reason} ->
        Logger.error("ProjectMarketSavingsRealized.Subscriber: ✗ Failed: #{inspect(reason)}")
  ...
  def handle_info(msg, state) do
    Logger.debug("ProjectMarketSavingsRealized.Subscriber: Unexpected message: #{inspect(msg)}")
    {:noreply, state}
  end
  ...
  defp handle_event(_topic, event_data) do
    CortexIqProjections.ProjectMarketSavingsRealized.Projector.project_event(event_data)
  end
end
